...
struct SwiftUIPFinanceView: View {
        ...
        ZStack {
            ScrollView(showsIndicators: false) {
                ...
                VStack {
                    ...
                    PFTransactionHeader(listType: $listType)
                        .padding(.bottom)
                }
                
                // List the transaction records
                ForEach(paymentDataForView) { transaction in
                    PFTransactionCellView(transaction: transaction)
                        .onTapGesture {
                ...
                .sheet(isPresented: self.$showPaymentDetails) {
                    if let selectedPaymentActivity = self.selectedPaymentActivity {
...
struct PFMenuBar<Content>: View where Content: View {
...
struct PFTransactionHeader: View {
...
struct PFTransactionCellView: View {
